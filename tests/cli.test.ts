...
async function captureConsoleOutput(func: () => void|Promise<void>): Promise<{stdout: string; stderr: string}> {
...
Deno.test("CLI should execute the greet command correctly", async () => {
    ...
    Object.defineProperty(Deno, 'args', { value: ["greet", "--name=Tester", "--verbose"] });
...
Deno.test("CLI should handle missing required flags", async () => {
    ...
    const { stderr: _stderr, stdout } = await captureConsoleOutput(async () => {
        try {
            await cli.run();
        } catch(e) {
            error = e as Error;
...
Deno.test("CLI should handle invalid flag values", async () => {
    ...
    // Mock Deno.args
    const originalArgs = Deno.args;
    Object.defineProperty(Deno, 'args', { value: ["greet", "--name=Bob", "--times=abc"] });
...
Deno.test("CLI should display general help when no command is provided", async () => {
    ...
    const { stderr: _, stdout } = await captureConsoleOutput(async () => {
        await cli.run();
    });
