...
export class FileSystemTracker {
	...
	static init() {
		...
		Deno.removeSync = function (
			pathToRemove: string | URL,
			...
			if (FileSystemTracker.isProtectedPath(resolvedPath)) {
				logger.warn(
					`Attempted to remove protected directory or file: ${resolvedPath}`,
				);
				throw new Error(
					`Protected directory removal attempted: ${resolvedPath}`,
				);
			}
		...
		Deno.remove = function (
			...
			if (FileSystemTracker.isProtectedPath(resolvedPath)) {
				...
				return Promise.reject(
					new Error(`Protected directory removal attempted: ${resolvedPath}`),
				);
			}
