...
Deno.test("Plugin Lifecycle Integration Tests", async (t) => {
    ...
    await t.step("plugin load and unload cycle", async () => {
        ...
        // Create plugin with corrected logging
            ...
            const plugin: Plugin = {
                ...
                init: async (cli: CLI) => {
                    try {
                        ...
                        cli.logger.error("Failed to initialize plugin:", error);
                        throw error;
                    }
                },
        ...
        try {
            ...
            // Test help to verify registration
            const helpResult = await framework.executeCommand(["help"]);
            assertEquals(helpResult.success, true, "Help command should work");
            ...
            assertEquals(postUnloadResult.success, false, "Command still available after unload");

            // Verify lifecycle logs with proper debug output
            const allLogs = [
                ...framework.getLogger().getLogs(),
                ...framework.getLogger().getDebugs()
            ];

            // Verify initialization logs
            const hasInitLog = allLogs.some(log => log.includes("Plugin initialized"));
            assertEquals(hasInitLog, true, `Missing initialization log. Logs: ${JSON.stringify(allLogs)}`);

            // Verify execution logs
            const hasExecLog = allLogs.some(log => log.includes("Test command executed"));
            assertEquals(hasExecLog, true, `Missing execution log. Logs: ${JSON.stringify(allLogs)}`);

            // Verify destruction logs
            const hasDestroyLog = allLogs.some(log => log.includes("Plugin destroyed"));
            assertEquals(hasDestroyLog, true, `Missing destruction log. Logs: ${JSON.stringify(allLogs)}`);
