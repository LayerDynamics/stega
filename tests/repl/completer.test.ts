...
Deno.test("CommandCompleter - path completion with temporary directories", async () => {
	const completer = new CommandCompleter([]);
...
Deno.test("CommandCompleter - getPathCompletions with temporary directories", async () => {
	const completer = new CommandCompleter([]);

	// Create temporary test directory structure
	const tempDir = await Deno.makeTempDir();
	await Deno.mkdir(`${tempDir}/repl`);
	await Deno.mkdir(`${tempDir}/core`);
	await Deno.writeTextFile(`${tempDir}/repl/fileA.ts`, "");
	await Deno.writeTextFile(`${tempDir}/core/fileB.ts`, "");

	try {
		const completionsRepl = await completer.getPathCompletions(`${tempDir}/re`);
		assertEquals(completionsRepl.length, 1);
		assert(completionsRepl.includes(`${tempDir}/repl`));

		const completionsCore = await completer.getPathCompletions(`${tempDir}/c`);
		assertEquals(completionsCore.length, 1);
		assert(completionsCore.includes(`${tempDir}/core`));
	} finally {
		await Deno.remove(tempDir, { recursive: true });
	}
});
