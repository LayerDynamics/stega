...
import {CommandPipeline,StringOperator,createPipelineEvaluator} from "../../src/repl/pipeline.ts";
...
Deno.test("StringOperator - transforms",async () => {
	...
	assertEquals(
		await StringOperator.transform("hello world","replace",["world","there"]),
		"hello there"
	);
...
Deno.test("CommandPipeline - complex pipeline",async () => {
	const pipeline=new CommandPipeline();
	pipeline.parse('echo "Hello, World!" | .lowercase | .replace "world" "there" | .trim');
...
Deno.test("CommandPipeline - quoted arguments",() => {
	const pipeline=new CommandPipeline();
	const stages=pipeline.parse('command "arg with spaces" \'another arg\' | command2');
