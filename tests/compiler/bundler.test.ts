...
Deno.test("Bundler - bundles modules correctly", async () => {
	...
	assertEquals(
		result.code.includes("export const a = 'a';"),
		true,
		"Bundled code should include module 'a.ts'",
	);
	assertEquals(
		result.code.includes("export const b = a + 'b';"),
		true,
		"Bundled code should include module 'b.ts'",
	);

	// Optionally, check if the preamble and postamble are present
	assertEquals(
		result.code.includes("__register"),
		true,
		"Bundled code should include __register function",
	);
	assertEquals(
		result.code.includes("__require"),
		true,
		"Bundled code should include __require function",
	);
});
