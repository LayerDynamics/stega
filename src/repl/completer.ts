...
export class CommandCompleter {
	...
	public async getPathCompletions(pathInput: string): Promise<string[]> {
		try {
			...
			const dirPath = normalizedInput ? path.dirname(normalizedInput) : ".";
			const prefix = normalizedInput
				? path.basename(normalizedInput).toLowerCase()
				: "";
	...
	public getFuzzyCompletions(input: string): string[] {
		...
		const regex = new RegExp(
			input.split("").map((char) => `(?=.*${char})`).join(""),
			"i",
		);
...
export function createCommandCompleter(cli: CLI): CommandCompleter {
	const commands = cli.getCommandRegistry().getCommands().map((cmd) =>
		cmd.name
	);
