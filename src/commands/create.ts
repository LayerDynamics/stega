...
import {Command,CLI} from "../core.ts";
import { promptString, promptConfirm } from "../prompts.ts";
...
export const createCommand: Command = {
    ...
    options: [
        ...
        { name: "force", alias: "f", type: "boolean", description: "Force creation" },
        { name: "output", alias: "o", type: "string", description: "Output format" },
    ],
    action: async (args) => {
        ...
        if (!force) {
            const confirm = await promptConfirm(cli.t("confirm_create_project", { name: projectName }));
            if (!confirm) {
