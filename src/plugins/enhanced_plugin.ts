...
export class PluginManager extends EventTarget {
    ...
    private dispatchPluginEvent(type: string, pluginName: string, error?: Error, config?: Record<string, unknown>): void {
        this.dispatchEvent(new CustomEvent<PluginEvent["detail"]>(type, {
            detail: {
                pluginName,
                error,
                config
    ...
    public async loadPlugin(
        ...
        try {
            ...
            if(!this.checkVersionCompatibility(plugin.metadata)) {
                throw new Error(`Plugin ${plugin.metadata.name} is not compatible with current runtime`);
            }
     ...
     */
    private async checkDependencies(metadata: EnhancedPluginMetadata): Promise<void> {
     ...
     */
    private async validateConfigSchema(schema: Record<string,unknown>): Promise<void> {
        // Implement JSON schema validation here
    }
     ...
     */
    public getAllPluginsStatus(): Record<string,ReturnType<typeof this.getPluginStatus>> {
